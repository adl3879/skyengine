...
namespace sky
{
CameraFrustum::CameraFrustum(const glm::mat4 &viewMatrix, const glm::mat4 &projMatrix)
{
    CalculateFrustum(viewMatrix * projMatrix);
}

CameraFrustum::CameraFrustum(const glm::mat4 &viewProjectionMatrix) { CalculateFrustum(viewProjectionMatrix); }

const glm::vec4 &CameraFrustum::getSide(const CameraFrustum::Side &side) const
{
    return m_Frustum[static_cast<std::uint32_t>(side)];
}

const std::array<glm::vec4, 6> &CameraFrustum::getSides() const { return m_Frustum; }

bool CameraFrustum::isInFrustum(const glm::mat4 &transform, const glm::vec3 &minHalfExt,
                                       const glm::vec3 &maxHalfExt)
{
    return SSE_OBBTest(transform, minHalfExt, maxHalfExt);
}

bool CameraFrustum::isInFrustum(const glm::vec3 &position, float radius)
{
    return Sphere_Test(position, radius);
}

bool CameraFrustum::isInFrustum(const glm::vec3 &position, const glm::vec3 &direction, float length,
                                       float radius)
{
    // Only apex works !
    ...
    for (const glm::vec4 &plane : m_Frustum)
    {
        ...
        if (dist1 < 0.0f && dist2 < 0.0f && dist3 < 0.0f && dist4 < 0.0f) return (isApexInside || false);
    }
...
void CameraFrustum::CalculateFrustum(const glm::mat4 &viewProjectionMatrix)
{
    ...
    // Normalizing x, y, z
    for (auto i = 0; i < 6; i++) Normalize(m_Frustum[i]);
}

void CameraFrustum::Normalize(glm::vec4 &side)
{
    /* Only for x, y, z*/
...
bool CameraFrustum::AABBTest(const glm::vec3 &minHalfExt, const glm::vec3 &maxHalfExt)
{
    bool inside = true;
    for (int i = 0; i < 6; i++)
    {
        float d = std::max(minHalfExt.x * m_Frustum[i].x, maxHalfExt.x * m_Frustum[i].x) +
                  ...
                  std::max(minHalfExt.z * m_Frustum[i].z, maxHalfExt.z * m_Frustum[i].z) + m_Frustum[i].w;
        inside &= d > 0;
    }
    return inside;
}

bool CameraFrustum::SSE_AABBTest(const glm::vec3 &minHalfExt, const glm::vec3 &maxHalfExt) { return false; }

bool CameraFrustum::OBBTest(const glm::mat4 &transform, const glm::vec3 &minHalfExt, const glm::vec3 &maxHalfExt)
{
    // Calculates the clip space matrix, which is the result of multiplying the View-Projection matrix by the transform
    // matrix.
    ...
    glm::vec4 points[8]{
        {minHalfExt.x, maxHalfExt.y, minHalfExt.z, 1.f}, {minHalfExt.x, maxHalfExt.y, maxHalfExt.z, 1.f},
        {maxHalfExt.x, maxHalfExt.y, maxHalfExt.z, 1.f}, {maxHalfExt.x, maxHalfExt.y, minHalfExt.z, 1.f},
        {maxHalfExt.x, minHalfExt.y, minHalfExt.z, 1.f}, {maxHalfExt.x, minHalfExt.y, maxHalfExt.z, 1.f},
        {minHalfExt.x, minHalfExt.y, maxHalfExt.z, 1.f}, {minHalfExt.x, minHalfExt.y, minHalfExt.z, 1.f}};

    // Applies the clip space matrix to every point in the array of points.
    for (glm::vec4 &point : points) point = clipSpaceMatrix * point;
...
bool CameraFrustum::SSE_OBBTest(const glm::mat4 &transform, const glm::vec3 &minHalfExt,
                                       const glm::vec3 &maxHalfExt)
{
    ...
    __m128 outside_negative_plane = _mm_set1_ps(-1.f); // because _mm_movemask_ps (while storing result) cares about
                                                       // 'most significant bits' (it is sign of float value)
    ...
    for (auto i = 0u; i < 8; i++)
    {
        ...
        // gather w & -w
        __m128 wwww =
            _mm_shuffle_ps(obb_transformed_point.xyzw, obb_transformed_point.xyzw, _MM_SHUFFLE(3, 3, 3, 3)); // get w
...
bool CameraFrustum::Sphere_Test(const glm::vec3 &position, float radius)
{
    bool result = true;
    for (const auto &plane : m_Frustum)
    {
        if (plane.x * position.x + plane.y * position.y + plane.z * position.z + plane.w <= -radius) result = false;
    }
    return result;
}
}