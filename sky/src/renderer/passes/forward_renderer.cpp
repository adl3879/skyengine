...
void ForwardRendererPass::init(const gfx::Device &device) 
{
    ...
    const auto layouts = std::array{device.getGpuSceneDescriptorLayout()};
    ...
    const std::vector<MeshDrawCommand> &drawCommands)
{
    ...
    const auto drawExtent = device.getDrawImageExtent();
    ...
    for (const auto &drawCommand : drawCommands)
	{
		...
		const auto pushConstants = PushConstants{
			.worldMatrix = drawCommand.modelMatrix,
			.vertexBuffer = mesh.vertexBufferAddress,
		};

		vkCmdPushConstants(cmd, m_pInfo.pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(PushConstants), &pushConstants);
