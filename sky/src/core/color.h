...
struct LinearColor
{
    ...
    static LinearColor fromRGB(std::uint8_t r, std::uint8_t g, std::uint8_t b, std::uint8_t a = 255);
...
{
    return gamma < 0.04045f ? gamma / 12.92f : std::pow(maxF(gamma + 0.055f, 0.f) / 1.055f, 2.4f);
}

// See http://www.brucelindbloom.com/index.html?Eqn_XYZ_to_RGB.html
float linearToGamma(float v) { return v < 0.00313108f ? 12.92f * v : 1.055f * std::pow(v, 1.f / 2.4f) - 0.055f; }
...
inline LinearColor rgbToLinear(const RGBColor &color) { return rgbToLinear(color.r, color.g, color.b, color.a); }
