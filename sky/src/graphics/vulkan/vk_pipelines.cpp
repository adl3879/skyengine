...
VkPipeline PipelineBuilder::build(VkDevice device)
{
    ...
    VkPipeline pipeline;
    const auto res = vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &pipeline);
    if (res != VK_SUCCESS)
    {
...
PipelineBuilder &PipelineBuilder::setShaders(VkShaderModule vertexShader, VkShaderModule geometryShader,
...
PipelineBuilder &PipelineBuilder::enableBlending(VkBlendOp blendOp, VkBlendFactor srcBlendFactor,
                                                 VkBlendFactor dstBlendFactor, VkBlendFactor srcAlphaBlendFactor,
